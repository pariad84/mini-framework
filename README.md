...
Every example defines every layout it needs (`popup`/`close-btn`/`save-btn`/`form` at minimum, plus `list`/`pagination` where used) in its own `layout.js`, loaded right after `fn.util.js` and before `app.js` -- none of them load `fn.component.layout.js`, which is itself just a reference implementation of those same layouts, not a framework dependency (see "Example folder structure" in CLAUDE.md). All four do load `fn.util.js`: `save-btn` calls `fn.util.saveForm`, which calls the popup's `.__form`'s own `.save()` (insert-or-update, defined identically in every example's `form` layout) and then refreshes the caller, passing only how that example's chrome closes (`popup.remove()` in `crm/`/`team-chat/`, `closeWindow` in `windows-os/`, `popScreen` in `android-phone/`); `app.js`'s add-buttons and resource-list-fetching both go through `fn.util.newButton`/`fn.util.selectFlat` instead of each defining its own.
