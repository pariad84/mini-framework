...
for later use. Example apps live in their own folders at the repo root, listed from `index.html`
-- currently `crm/`, `windows-os/`, and `android-phone/`.
...
- **Example folder structure**: `index.html` + `layout.js` + `app.js`, always, loaded in that
  order after `fn.js`. `fn.js` alone doesn't register any layouts, so `layout.js` must define
  every one the app needs -- at minimum `popup`/`close-btn`/`save-btn`/`form`/`list` (and
  `pagination`, if a list can grow past `pageSize`); copy from `fn.component.layout.js` as a
  starting point and re-theme only what the example's look needs to change (see `crm/`,
  `windows-os/`, `android-phone/`). Always name that file `layout.js` — not
  `mobile-layout.js`/`desktop-chrome.js`/etc. — so every example's layouts live in a file with
  the same name.
...
1. Implement the change (in `fn.js` if it's the framework, in `fn.component.layout.js` if it's
   the reference layout implementation, or in whatever app you're building on top of it if it's
   app-specific).
...
3. Verify in an actual browser (Playwright) — load the relevant example's HTML file (e.g.
   `crm/index.html`), or build a scratch page loading `fn.js` then `fn.component.layout.js` (or
   a scratch `layout.js` of your own) if no example covers it, and drive it with Playwright —
   check the result (DOM state, localStorage). This project has no committed test suite, so
   this is the only real verification available before committing.
4. Commit and push.
